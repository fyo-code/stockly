"""Phase 8G-P final promotion pack for forecast v2 high-revenue policy."""
...
try:
    ...
    from .sklearn_direct_model import (
        ...
        HIGH_REVENUE_GUARDED_CAMPAIGN_SOURCE,
        HIGH_REVENUE_GUARDED_CAMPAIGN_VERSION,
        run_sklearn_models,
    )
...
DEFAULT_REPORT = PROJECT_ROOT / "active_docs" / "ITER5AE_V2_PHASE8G_P_FINAL_PROMOTION_PACK.md"
DEFAULT_SCORE_ROWS_CSV = PROJECT_ROOT / "active_docs" / "ITER5AE_V2_PHASE8G_P_OFFICIAL_SCORE_ROWS.csv"
...
GUARD_WINDOWS = ["2024-12-30", "2025-01-27"]
...
def _aggregate_rows(score_rows: pd.DataFrame) -> list[list[str]]:
    ...
    order = {
        ...
        HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL: 2,
        "sk_extra_trees": 3,
        "sk_hgb_squared": 4,
...
def _critical_rows(score_rows: pd.DataFrame, candidate_model: str, baseline_model: str) -> list[list[str]]:
    ...
    slices = [
        ...
        ("2024-12-30 guard window", score_rows["target_start"].astype(str) == "2024-12-30"),
        ("2025-01-27 guard window", score_rows["target_start"].astype(str) == "2025-01-27"),
        ("2025-03-24 monitor window", score_rows["target_start"].astype(str) == "2025-03-24"),
    ]
    return [_compare_row(score_rows, candidate_model, baseline_model, label, mask) for label, mask in slices]


def _route_rows(score_rows: pd.DataFrame, candidate_model: str, baseline_model: str) -> list[list[str]]:
    present = set(score_rows["primary_route"].dropna().astype(str))
    return [
        _compare_row(
            score_rows,
            candidate_model,
            baseline_model,
            route,
            score_rows["primary_route"].astype(str) == route,
        )
        for route in [route for route in ROUTE_ORDER if route in present]
    ]
...
def _zero_actual_rows(score_rows: pd.DataFrame) -> list[list[str]]:
    candidate = _model_metrics(score_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL)
    rows: list[list[str]] = []
...
def _max_non_stress_wmape_regression(score_rows: pd.DataFrame) -> tuple[str | None, float | None]:
    ...
    for target_start in sorted(score_rows["target_start"].dropna().astype(str).unique()):
        ...
        champion = _metrics(_model_mask(score_rows, HIGH_REVENUE_CHAMPION_MODEL, mask))
        candidate = _metrics(_model_mask(score_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, mask))
...
def _gate_rows(score_rows: pd.DataFrame) -> tuple[str, list[list[str]]]:
    ...
    candidate = _model_metrics(score_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL)
    top100_champion = _metrics(_model_mask(score_rows, HIGH_REVENUE_CHAMPION_MODEL, _scope_mask(score_rows, 100)))
    top100_candidate = _metrics(_model_mask(score_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, _scope_mask(score_rows, 100)))
    top500_champion = _metrics(_model_mask(score_rows, HIGH_REVENUE_CHAMPION_MODEL, _scope_mask(score_rows, 500)))
    top500_candidate = _metrics(_model_mask(score_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, _scope_mask(score_rows, 500)))
    stress_champion = _metrics(
        _model_mask(score_rows, HIGH_REVENUE_CHAMPION_MODEL, score_rows["target_start"].astype(str) == STRESS_WINDOW)
    )
    stress_candidate = _metrics(
        _model_mask(score_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, score_rows["target_start"].astype(str) == STRESS_WINDOW)
    )
    worst_window, worst_delta = _max_non_stress_wmape_regression(score_rows)
    checks = [
        ("Top 1000 hit +/-20 vs 8G-I", ">= +1.0pp", _delta(candidate["hit20"], champion["hit20"]), 0.010, "min"),
        ("Top 1000 WMAPE vs 8G-I", "<= 0.0pp", _delta(candidate["wmape"], champion["wmape"]), 0.0, "max"),
        ("Top 1000 bias vs 8G-I", "> 0.0pp", _delta(candidate["bias"], champion["bias"]), 0.0, "min_strict"),
        ("Top 1000 phantom vs 8G-I", "<= 0.0pp", _delta(candidate["phantom_rate"], champion["phantom_rate"]), 0.0, "max"),
        ("Top 500 hit +/-20 vs 8G-I", ">= 0.0pp", _delta(top500_candidate["hit20"], top500_champion["hit20"]), 0.0, "min"),
        ("Top 100 hit +/-20 vs 8G-I", ">= 0.0pp", _delta(top100_candidate["hit20"], top100_champion["hit20"]), 0.0, "min"),
        (f"{STRESS_WINDOW} WMAPE vs 8G-I", "<= 0.0pp", _delta(stress_candidate["wmape"], stress_champion["wmape"]), 0.0, "max"),
        (f"Largest non-stress WMAPE regression vs 8G-I ({worst_window or '-'})", "<= +2.0pp", worst_delta, 0.020, "max"),
        ("Top 1000 hit +/-20 vs control", ">= +1.5pp", _delta(candidate["hit20"], control["hit20"]), 0.015, "min"),
        ("Top 1000 WMAPE vs control", "<= +0.5pp", _delta(candidate["wmape"], control["wmape"]), 0.005, "max"),
    ]
    rows: list[list[str]] = []
    passes: list[bool] = []
    ...
    decision = "PROMOTE_8G_O_GUARDED_POLICY_AND_STOP_CURRENT_DATA_ITERATION" if all(passes) else "KEEP_8G_I_CHAMPION_AND_STOP_FOR_NEW_DATA"
    return decision, rows


def _policy_check_rows(no_policy_rows: pd.DataFrame, candidate_rows: pd.DataFrame, guard_passed: bool) -> list[list[str]]:
    no_policy_candidate_present = bool((no_policy_rows["model_name"] == HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL).any())
    candidate_present = bool((candidate_rows["model_name"] == HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL).any())
    ...
    return [
        ["Default policy emits guarded candidate", "no", "yes" if no_policy_candidate_present else "no", "PASS" if not no_policy_candidate_present else "FAIL"],
        ["Guarded policy emits guarded candidate", "yes", "yes" if candidate_present else "no", "PASS" if candidate_present else "FAIL"],
        ["Guarded policy includes 8G-I champion for comparison", "yes", "yes" if champion_present else "no", "PASS" if champion_present else "FAIL"],
        ["Guarded policy blocked without Top 1000-or-lower rank scope", "yes", "yes" if guard_passed else "no", "PASS" if guard_passed else "FAIL"],
        [
...
def build_report(
    ...
    candidate = _model_metrics(candidate_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL)
    promote = decision.startswith("PROMOTE")
    decision_note = (
        ...
        if promote
        else "The guarded pre-BF campaign candidate fails the final gates. Keep the 8G-I champion and stop current-data model iteration until new data arrives."
    )
    return "\n".join(
        [
            ...
            _table(
                ...
                [
                    ["Model", CONTROL_MODEL, HIGH_REVENUE_CHAMPION_MODEL, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, "-"],
            ...
            _table(
                ...
                _scope_rows(candidate_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, HIGH_REVENUE_CHAMPION_MODEL),
            ),
            "",
            ...
            _table(
                ...
                _critical_rows(candidate_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, HIGH_REVENUE_CHAMPION_MODEL),
            ),
            "",
            ...
            _table(
                ...
                _route_rows(candidate_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, HIGH_REVENUE_CHAMPION_MODEL),
            ),
            "",
            ...
            _table(
                ...
                _window_rows(candidate_rows, HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL, HIGH_REVENUE_CHAMPION_MODEL),
            ),
            "",
            ...
            _table(
                [
                    ...
                    "8G-O pred units on zero actual",
            ...
            f"- 8G-O guarded model/version: `{HIGH_REVENUE_GUARDED_CAMPAIGN_MODEL}` / `{HIGH_REVENUE_GUARDED_CAMPAIGN_VERSION}`.",
            f"- 8G-O prediction source: `{HIGH_REVENUE_GUARDED_CAMPAIGN_SOURCE}`.",
            f"- Official score-row CSV: `{score_rows_csv}`.",
...
def _guard_check(conn: sqlite3.Connection, target_starts: list[str], min_train_windows: int, random_state: int, config: ScorecardConfig) -> bool:
    try:
        run_sklearn_models(
            ...
            high_revenue_policy="pre_bf_bfc_lift_180",
            config=config,
            include_score_rows=True,
        )
    except ValueError:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Phase 8G-P final promotion pack.")
    ...
    if args.revenue_rank_limit != 1000:
        raise ValueError("Phase 8G-P validates the official Top 1000 policy scope: revenue_rank_limit must be exactly 1000")
